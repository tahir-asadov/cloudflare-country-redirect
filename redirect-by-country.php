...
function ccr_settings_page()
{
  ...
  if (isset($_POST['ccr_rules_submit']) && check_admin_referer('ccr_rules_form')) {
    // Sanitize inputs
    $countries = isset($_POST['country']) ? array_map('sanitize_text_field', $_POST['country']) : [];
    $urls = isset($_POST['url']) ? array_map('esc_url_raw', $_POST['url']) : [];
    ...
    for ($i = 0; $i < count($countries); $i++) {
      ...
      $u = trim($urls[$i]);
      if ($c && $u) {
        $rules[] = ['country' => $c, 'url' => esc_url_raw($u)];
      }
    }
